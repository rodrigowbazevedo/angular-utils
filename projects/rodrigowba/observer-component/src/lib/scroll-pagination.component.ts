...
import { Pagination, isInitialPagination, filterPaginationScrollPosition } from '@rodrigowba/http-common';
...
export abstract class ScrollPaginationComponent extends ObserverComponent implements OnInit {
    ...
    selectToLoadPage<T>(pagination$: Observable<Pagination>): Observable<T> {
        return merge<T>(
            pagination$.pipe(
